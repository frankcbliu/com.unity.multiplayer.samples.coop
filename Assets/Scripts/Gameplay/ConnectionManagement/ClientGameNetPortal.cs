...
namespace Unity.Multiplayer.Samples.BossRoom.Client {
  ...
  public class ClientGameNetPortal : MonoBehaviour {
    public static ClientGameNetPortal Instance; // 客户端实例
    ...
    private void InjectDependencies(
        ...
        IPublisher<QuitGameSessionMessage> quitGameSessionPub,
        IPublisher<ConnectStatus> connectStatusPub,
        IPublisher<ReconnectMessage> reconnectMessagePub
        ) {
    ...
    public void OnUserDisconnectRequest() {
      if (m_Portal.NetManager.IsClient) {
        ...
        if (m_TryToReconnectCoroutine != null) {
          ...
          m_ReconnectMessagePub.Publish(
              new ReconnectMessage(k_NbReconnectAttempts, k_NbReconnectAttempts)
          );
        }
    ...
    private void OnDisconnectOrTimeout(ulong clientID) {
      ...
      if (
          !NetworkManager.Singleton.IsHost // 非主机，本机调用，不需要判断clientID
          || NetworkManager.Singleton.IsHost // 主机因为客户端的调用也会唤起该程序，需要判断是否主机调用
        ...
        switch (DisconnectReason.Reason) {
          ...
          case ConnectStatus.IncompatibleBuildType:
            ...
            m_QuitGameSessionPub.Publish(
                new QuitGameSessionMessage() { UserRequested = false }
      ...
      while (nbTries < k_NbReconnectAttempts) {
        ...
        yield return new WaitForSeconds(
            1.1f * NetworkManager.Singleton.NetworkConfig.ClientConnectionBufferTimeout
                ...
                ).DisconnectTimeoutMS / 1000.0f
        ); // wait a bit longer than the timeout duration to make sure we have enough time to stop this coroutine if successful
        nbTries++;
      }
    ...
    public async Task StartClientUnityRelayModeAsync(Action<string> onFailure) {
      ...
      try {
        var (
            ...
            hostConnectionData,
            key
        ) = await UnityRelayUtilities.JoinRelayServerFromJoinCode(
            m_LocalLobby.RelayJoinCode
        );

        await m_LobbyServiceFacade.UpdatePlayerRelayInfoAsync(
            allocationIdBytes.ToString(),
            m_LocalLobby.RelayJoinCode
        );
        var utp = (UnityTransport)NetworkManager.Singleton.NetworkConfig.NetworkTransport;
    ...
    public static void ReceiveServerToClientConnectResult_CustomMessage(
        ulong clientID,
